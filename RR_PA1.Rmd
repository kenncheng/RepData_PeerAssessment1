...
The mean number of steps taken per day is `r rmean` and median number of steps taken per day is `r rmedian`.
...
max_steps
```
The `r max_interval`th 5-minute interval contains the maximum number of `r max_steps` steps.
...
median_i
```
The mean is `r mean_i` and median is `r median_i` for the total number of steps taken per day in the imputed data.
...
median_diff
```
The difference in the mean of imputed data from original data is `r mean_diff`.
The difference in the median of imputed data from original data is `r median_diff`.
...
The imputed data set resulted in `r total_diff` more total daily number of steps.
